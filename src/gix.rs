...
pub fn get_repo_description(manifest_location: &path::Path) -> Option<(String, bool)> {
...
/// or if any operation on the repository fails, `None` is returned.
#[cfg_attr(feature = "git2", allow(unused))]
pub fn get_repo_head(manifest_location: &path::Path) -> Option<(Option<String>, String, String)> {
    let repo = gix::discover(manifest_location).ok()?;
